...
class Constants:
    ...
    def calculate_constants(self):
        ...
        c14_list = self.calculate_c14_list()
        self.constants["c14_list"] = c14_list
        ...
        eqn1_bound = self.calculate_eqn(c14_list,c5)
        self.constants["eq_n1_bound"] = eqn1_bound
        
        eqz_bounds = self.calculate_eqz(eqn1_bound,c14_list)
        self.constants["eq_Z_bounds"] = eqz_bounds
    ...
    def calculate_c14_list(self):
        c14_list = []
        for i in range(len(self.primes)):
            C1p = self.calculate_bugeaud_constant(self.primes[i])
            hp1 = max(math.log(abs(self.a)), math.log(abs(self.b)), math.log(max(self.primes)))
            hp2 = max(math.log(abs(self.alpha)), math.log(abs(self.beta)), math.log(max(self.primes)))
            c14_list.append(C1p*hp1*hp2 + self.log_star(abs(self.alpha))/math.log(self.primes[i]))
        return c14_list
    
    def calculate_eqn(self, c14_list, c5):
        v = sum([c14_list[i]*math.log(self.primes[i]) for i in range(len(self.primes))])/math.log(abs(self.alpha))
        term1 = 4*(math.sqrt(c5) + math.sqrt(v)*math.log(4*v))**2
        term2 = 4*(math.sqrt(c5) + 2*math.e**2)**2
        return max(term1,term2)
    
    def calculate_eqz(self, eqn, c14_list):
        zb_list = []
        for i in range(len(self.primes)):
            zb_list.append(c14_list[i]*max(math.log(eqn),10*math.log(self.primes[i]),10)**2)
        return zb_list
...
if __name__ == "__main__":
    ...
    n1_bound = constants.calculate_eqn(c["c14_list"], c["c5"])
    print(n1_bound)
    zbound = constants.calculate_eqz(n1_bound,c["c14_list"])
    print(zbound)